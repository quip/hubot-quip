...
function Client(options) {
    ...
    this.baseUrl = options.baseUrl || "https://platform.quip.com";
}
...
Client.prototype.getAuthorizationUrl = function(redirectUri, state) {
    return this.baseUrl + '/1/oauth/login?' + querystring.stringify({
 ...
 * @param {function(Error, Object)} callback
 */
Client.prototype.getWebsocket = function(callback) {
...
Client.prototype.call_ = function(path, callback, postArguments) {
    var requestOptions = url.parse(this.baseUrl);
    requestOptions['path'] = '/1/' + path;
    ...
    var request = (requestOptions.protocol == "https:" ? https : http).request(
        requestOptions, function(response) {
            var data = [];
